      ...
      Object.keys(schema.paths).forEach((pathKey) => {
        ...
        Object.keys(_.pick(path, constants.httpMethods)).forEach((operationKey) => {
            ...
            operation.parameters.forEach((parameter, parameterIndex) => {
              if (parameter.in === 'path') {
                if (!_.contains(pathTemplateParameters, parameter.name)) {
                  errorList.push(new RuleFailure({ location: `paths.${pathKey}.${operationKey}.parameters[${parameterIndex}]`, hint: `missing from path template: ${parameter.name}` }));
                } else {
                  // remove validated param name
                  extraPathTemplateParameters = _.without(extraPathTemplateParameters, parameter.name);
                }
              }
            });
          }
          ...
          extraPathTemplateParameters.forEach((parameter) => {
            errorList.push(new RuleFailure({ location: `paths.${pathKey}.${operationKey}`, hint: `missing from parameter list: ${parameter}` }));
          });
        });
      });
    }
