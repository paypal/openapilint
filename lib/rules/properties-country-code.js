 ...
 * @param {Object} rootSchema The original schema this rule is checking.
 * @param {Object} errorList The local list of errors.
 * @returns {Object} dereferencedSchema The derefenced schema.
 */
function getReferencedSchema(schemaObject, pathToSchema, rootSchema, errorList) {
  if (schemaObject.$ref.startsWith('#/')) {
    const trimmedRef = schemaObject.$ref.substr(2);
    const splitRef = trimmedRef.split('/');

    // assume definitions are only 2 levels deep.
    const referencedSchema = rootSchema[splitRef[0]][splitRef[1]];
 ...
 * @param {Object} errorList The local list of errors.
 */
function checkSchemaObject(schemaObject, pathToSchema, rootSchema, visitedRefs, errorList) {
  if (!schemaObject) {
    return;
  }
  ...
  if (schemaObject.$ref && !_.includes(visitedRefs, schemaObject.$ref)) {
    ...
    const referencedSchema = getReferencedSchema(schemaObject, pathToSchema, rootSchema, errorList);

    checkSchemaObject(referencedSchema, pathToSchema, rootSchema, visitedRefs, errorList);
    ...
    Object.keys(schemaObject.properties).forEach((propertyKey) => {
      ...
      if (propertyInfo.$ref && !_.includes(visitedRefs, propertyInfo.$ref)) {
        visitedRefs.push(schemaObject.$ref);
        const referencedSchema = getReferencedSchema(
          propertyInfo, pathToSchema, rootSchema, errorList);

        checkSchemaObject(referencedSchema, pathToSchema, rootSchema, visitedRefs, errorList);
      }
    });
  } else if (schemaObject.type === 'array' && schemaObject.items) {
    checkSchemaObject(schemaObject.items, `${pathToSchema}.items`, rootSchema, visitedRefs, errorList);
  }

  if (schemaObject.allOf) {
    schemaObject.allOf.forEach((allOfValue, allOfIndex) => {
      checkSchemaObject(allOfValue, `${pathToSchema}.allOf[${allOfIndex}]`, rootSchema, visitedRefs, errorList);
    });
  }
}
        ...
        Object.keys(_.pick(path, constants.httpMethods)).forEach((operationKey) => {
          ...
          if (operation.parameters) {
            operation.parameters.forEach((parameter, parameterIndex) => {
              checkSchemaObject(parameter.schema, `paths.${pathKey}.${operationKey}.parameters[${parameterIndex}].schema`, rootSchema, visitedRefs, errorList);
            });
          }
          ...
          if (operation.responses) {
            Object.keys(operation.responses).forEach((responseKey) => {
              ...
              checkSchemaObject(response.schema, `paths.${pathKey}.${operationKey}.responses.${responseKey}.schema`, rootSchema, visitedRefs, errorList);
            });
          }
        });
      });
