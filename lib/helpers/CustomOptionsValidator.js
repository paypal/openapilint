 ...
 */
function isValidPatternOption(pattern, patternIgnoreCase) {
...
class CustomOptionsValidator {
  ...
  validateOptions() {
    ...
    getNormalizedOptions(this.options).forEach((option) => {
      if (!option.whenField
      || (!isValidPatternOption(option.thenPattern, option.thenPatternIgnoreCase))
      || !option.thenField
      || (!isValidPatternOption(option.whenPattern, option.whenPatternIgnoreCase))) {
