...
class CustomOptionsValidator {
   ...
   * @param {Function} predicateFunction An optional function to let the calling method
   *                   run an additional check.
   */
  validateAllCustoms(key, object, pathToObject, objectName, predicateFunction) {
    getNormalizedOptions(this.options).forEach((option) => {
      const shouldExecute = (predicateFunction === undefined
        || predicateFunction(option));
