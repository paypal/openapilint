...
class SchemaObjectParser {
   ...
   */
  checkSchemaObject(schemaFunction, propertyFunction, schemaObject, pathToSchema) {
    if (!schemaObject) {
      return;
    }
    ...
    if (schemaObject.$ref) {
      ...
      if (schemaObject.$ref.startsWith('#/definitions/')) {
        ...
        this.checkSchemaObject(schemaFunction, propertyFunction, referencedSchema, pathToSchema);
      } else {
      ...
      if (schemaObject.type === 'object' || schemaObject.isAllOf) {
        if (schemaObject.properties && propertyFunction) {
          Object.keys(schemaObject.properties).forEach((propertyKey) => {
            ...
            this.checkSchemaObject(schemaFunction, propertyFunction, propertyObject, pathToSchema);
          });
        }
      } else if (schemaObject.type === 'array' && schemaObject.items) {
        this.checkSchemaObject(schemaFunction, propertyFunction, schemaObject.items, `${pathToSchema}.items`);
      }
    }
    ...
    if (schemaObject.allOf) {
      schemaObject.allOf.forEach((allOfValue, allOfIndex) => {
        ...
        implicitAllOfValue.isAllOf = true;
        this.checkSchemaObject(schemaFunction, propertyFunction, implicitAllOfValue, `${pathToSchema}.allOf[${allOfIndex}]`);
      });
    }
  }

  forEachSchema(schemaFunction, propertyFunction) {
    this.visitedRefs = [];
        ...
        Object.keys(_.pick(path, constants.httpMethods)).forEach((operationKey) => {
          ...
          if (operation.parameters) {
            operation.parameters.forEach((parameter, parameterIndex) => {
              this.checkSchemaObject(schemaFunction, propertyFunction, parameter.schema, `paths.${pathKey}.${operationKey}.parameters[${parameterIndex}].schema`);
            });
          }
          ...
          if (operation.responses) {
            Object.keys(operation.responses).forEach((responseKey) => {
              ...
              this.checkSchemaObject(schemaFunction, propertyFunction, response.schema, `paths.${pathKey}.${operationKey}.responses.${responseKey}.schema`);
            });
          }
        });
      });
